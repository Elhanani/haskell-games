...
module PQueueQuick (PQueue, insert, extract, fromList, toAscList) where
...
data ViewQ a = EmptyQ    -- ^ empty queue
  | a :^ PQueue a    -- ^ the top (minimum) of the queue and the rest of the queue
...
foldrAsc f z (PQueue n h) = foldrHeap (<=) f z h
foldrAsc _ z _ = z
